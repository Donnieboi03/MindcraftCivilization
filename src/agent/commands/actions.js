...
import {BLUEPRINTS} from '../npc/construction/index.js';
...
export const actionsList = [
    {
        name: '!construct',
        description: 'Builds a specified structure near the bot.',
        params: {
            // Choose Structure
            structure: {type: 'string', description: `Which structure to build ${Object.keys(BLUEPRINTS).join(', ')}`}
        },
        perform: async function (agent, structure) {
            let result = '';
            const actionFn = async () => {
                try{
                    const info = await skills.buildFromBlueprint(agent.bot, BLUEPRINTS[structure.toLowerCase()]);
                    result = `Built ${info.name} at ~x:${info.origin.x}, y:${info.origin.y}, z:${info.origin.z}.`;
                } catch(e){
                    result =  `Error in construct: ${e.toString()}`;
                }
            };
            await agent.actions.runAction('action:construct', actionFn, { timeout: 15 });
            return result;
        }
    },
    {
